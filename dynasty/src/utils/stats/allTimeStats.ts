...
export const getAllTimeRosterStats = (rID: number, legacyLeague: Interfaces.League[]) => {
    const legacyRosters = legacyLeague
    .map(league => league.rosters
    .find(roster => roster.roster_id === rID));
 
    const legacyMatches: Interfaces.Match[][] = legacyLeague
    .map(league => league.matchups
    ...
    const allTimeRegularSeasonWins = legacyRosters
    .reduce((acc, item: any) => acc + item?.settings?.wins, 0);
    
    const allTimeRegularSeasonLosses = legacyRosters
    .reduce((acc, item: any) => acc + item?.settings?.losses, 0);
    
    const allTimeRegularSeasonTies = legacyRosters
    .reduce((acc, item: any) => acc + item?.settings?.ties, 0);
    
    const allTimeRegularSeasonFPTS = roundToHundredth(legacyRosters?.reduce((acc, item: any) =>  acc + Number(item?.settings?.fpts + "." + (item?.settings?.fpts_decimal || 0)), 0));
    const allTimeRegularSeasonPPTS = roundToHundredth(legacyRosters?.reduce((acc, item: any) =>  acc + Number((item?.settings?.ppts || 0) + "." + (item?.settings?.ppts_decimal || 0)), 0));
    const allTimeRegularSeasonPA   = roundToHundredth(legacyRosters?.reduce((acc, item: any) =>  acc + Number((item?.settings?.fpts_against || 0) + "." + (item?.settings?.fpts_against_decimal || 0)), 0));
    ...
    const bestScore = legacyMatches?.map(match => match
    ...
    const playoffRuns: Interfaces.PlayoffRuns[] = legacyLeague.map(league => {
        const playoffBracket = league.brackets.playoffs
        .filter(match => match.t1 === rID || match.t2 === rID);
        ...
        if (playoffBracket.length > 0) {
            ...
            return {
                ...
                season:  league.season,
                games:   Number(league.season) > 2020 ? 
                weeklyScore?.slice(14,17) : weeklyScore?.slice(13,16)
            };
        };
        return {
            bracket: [],
    ...
    const calculatePlayoffStreak = (data: any) => {
    ...
    const totalPlayoffPA:   number = roundToHundredth(
        playoffAppearances > 0 ?
        playoffRuns.map((season) =>
        season?.games.map((game) => game
        .filter((team) => team.roster_id !== rID)[0])
        .map((team) => team && team.points)
        .reduce((a, b) => (a !== undefined && b !== undefined ? +a + +b : a), 0))
        .reduce((a, b) => (a !== undefined && b !== undefined ? +a + +b : a), 0)
    : 0);
    
    const highestPlayoffScore: number = roundToHundredth(
    playoffAppearances > 0 ? 
    playoffRuns.map(season => season.games
    .map(game => game.filter(team => team.roster_id === rID)[0])
    .map(team => team && team.points)
    .sort((a,b) => b - a)[0])
    .sort((a,b) => b - a)[0] : 0);

    const totalPlayoffGames = playoffAppearances > 0 ? 
    playoffRuns?.map(season => season.bracket.length)
    .reduce((a,b) => {return +a + +b}) : 0;
    
    const allTimePlayoffWins = playoffRuns?.map(season => season.bracket
    .filter(match => match.w === rID))?.map(szn => szn.length).reduce((acc, n) => acc + n, 0) || 0; 
    
    const allTimePlayoffLosses = playoffRuns?.map(season => season.bracket
    .filter(match => match.l === rID))?.map(szn => szn.length).reduce((acc, n) => acc + n, 0) || 0;
    
    // Top Scoring Players 
    const topPlayerScores = legacyLeague.slice().map((league) => {
        const addedWeekLabel = league.matchups
        .map((matchup: Interfaces.Match[], i) => { 
            return {
                ...matchup.find(team => team.roster_id === rID), week: i + 1 
            }
    ...
    }).flat().sort((a, b) => b.points - a.points).map((record, i)=> { 
        return {
            ...record, 
            rank: i + 1
        }
    ...
    const topTeamScores: Interfaces.HighScoreRecord[] = legacyLeague.slice().map(league => {
        const myMatches = league.matchups.map((matchup, i) => {
            ...
            const opponent = matchup.find(match => match.roster_id !== rID 
            && myTeam?.matchup_id === match.matchup_id);
    ...
    const longestWinStreak = seasonalStreaks
    .reduce((max, current) => (current.wins > max.wins ? current : max), seasonalStreaks[0]);

    const longestLosingStreak = seasonalStreaks
    .reduce((max, current) => (current.losses > max.losses ? current : max), seasonalStreaks[0]);
...
export const getAllTimeLeagueStats = (legacyLeague: Interfaces.League[]) => {
    ...
    const unsortedList = legacyLeague.slice().map((league) => {
        const recordLabels = league.matchups
        .map((match) => match.filter(matchup => matchup.matchup_id !== null))
        .filter(matchups => matchups.length > 0).map((matchups, idx) =>
        matchups.map(match => {
    ...
    const topPlayerScorerList: Interfaces.HighScoreRecord[] = unsortedList.flat()
    .sort((a, b) => b.points - a.points).map((_, idx) => {
        return {
            ..._, 
            rank: idx + 1
        }
    ...
    }).flat().sort((a, b) => b?.points! - a?.points!).map((_, idx) => {
        return {
            ..._, 
            rank: idx + 1
        }
