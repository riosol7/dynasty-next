...
export const getPowerRankings = (season: string, legacyLeague: Interfaces.League[]) => {
    if (season === "All Time") {
        ...
        }).slice().sort((a, b) => b.settings.all_play_win_rate - a.settings.all_play_win_rate)
        .map((roster, i) => {
            return {
                ...roster, 
                power_rank: i + 1
...
export const getAllPlayStats = (rID: number, season: string, legacyLeague: Interfaces.League[]) => {
    ...
    if (season === "All Time") {
        const legacyMatches = legacyLeague.map(league => 
            Number(league.season) > 2020 ?
                league.matchups.slice(0, 14) // 17 game season
            :
                league.matchups.slice(0, 13) // 16 game season
        );
        ...
        const matches     = Number(foundLeague?.season) > 2020 ? 
            foundLeague?.matchups.slice(0, 14) : foundLeague?.matchups.slice(0, 13);
    ...
    const seasonalAllPlayRecord = seasonalRecord.reduce((prev, current) => {
        return { 
            wins:   prev.wins + current.wins, 
            losses: prev.losses + current.losses 
        };
    }, { 
        wins: 0, 
        losses: 0 
    ...
    const allTimeAllPlayRecord = historicalRecord.reduce((prev, current) => {
        return { 
            wins:   prev.wins + current.wins, 
            losses: prev.losses + current.losses 
        };
