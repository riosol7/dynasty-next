...
// matches: Interfaces.Match[][]

export const getTotalPts = (legacyLeague: Interfaces.League[], rID: number, pID: string) : { pts: number, maxPts: number } => {

    const pts = legacyLeague.map((league: Interfaces.League) => league.matchups.map(week => week.filter(team => team.roster_id === rID)[0]?.starters.find((starter: string) => starter === pID) !== undefined ? 
